...
class SkudonetAPIScriptWriter:
    """
    Generates a bash shell script (skudonet_apply.sh) or PowerShell script 
    (skudonet_apply.ps1) of API calls that call the Skudonet ZAPI v4.0 REST API 
    ...
    def __init__(self, mapper: SkudonetMapper, output_dir: str, format: str = "bash"):
        self.mapper     = mapper
        self.output_dir = output_dir
        self.format     = format
    ...
    # ------------------------------------------------------------------
    def write(self) -> str:
        """Write skudonet_apply.sh or skudonet_apply.ps1 and return the absolute file path."""
        self._write_header()
        ...
        if self.format == "powershell":
            filename = "skudonet_apply.ps1"
        else:  # default to bash
            filename = "skudonet_apply.sh"
    ...
    def _curl(self, method: str, endpoint: str,
        ...
        if self.format == "powershell":
            # PowerShell syntax
            self._w(f"$RESP = Invoke-WebRequest -Uri \"$env:BASE_URL{self.ZAPI}{endpoint}\"")
            self._w(f"  -Method {method} ")
            self._w(f"  -Headers @{{'ZAPI_KEY' = \"$env:API_KEY\"; 'Content-Type' = 'application/json'}}")
            self._w(f"  -Body '{body}'")
            self._w(f"check_response $RESP.Content \"{label}\"")
        else:
            # Bash syntax (original)
    ...
    def _write_header(self):
        if self.format == "powershell":
            self._w("#!/usr/bin/env pwsh")
            self._w("# ==========================================================================")
            self._w("# skudonet_apply.ps1")
            ...
            self._w("# -------------")
            self._w("# 1. Set BASE_URL:     $env:BASE_URL=\"https://your-skudonet-host\"")
            self._w("# 2. Set API_KEY:      $env:API_KEY=\"your-api-key\"")
            ...
            self._w("# USAGE")
            self._w("# -----")
            self._w("#   pwsh skudonet_apply.ps1")
            self._w("#   $env:BASE_URL=\"https://10.0.0.1\" $env:API_KEY=\"mykey\" pwsh skudonet_apply.ps1")
            self._w("#")
            self._w()
            self._w("function check_response() {")
            self._w("  param($resp, $label)")
            self._w("  if ($resp -match '\"error\"|\"description\":.*[Ee]rror') {")
            self._w("    Write-Host \"[FAIL] $label\"")
            self._w("    Write-Host \"       $resp\" | Select-Object -First 10")
            self._w("    if ($env:ABORT_ON_ERROR -eq '1') { exit 1 }")
            self._w("  } else {")
            self._w("    Write-Host \"[ OK ] $label\"")
            self._w("  }")
            self._w("}")
            self._w()
        else:
            # Original bash header
...
def main():
    ...
    if not args.dry_run:
        ...
        sh_path   = SkudonetAPIScriptWriter(mapper, args.output_dir, args.api_format).write()
